...
def get_llm_with_fallback():
    ...
    return LLM(
        ...
        max_tokens=8000,  # Increased to allow comprehensive analysis
        timeout=180,  # 3 minutes timeout for thorough analysis
...
class LocalizeAgent:
    ...
    def code_analyzer_agent(self) -> Agent:
        # No tools here — metrics are already pre-computed by batch_analyzer.py
        # and passed via {metrics} in the task description.
        # Giving tools caused the agent to re-call them 9 times, overflowing
        # Haiku's context window and producing empty LLM responses.
        return Agent(
